 ...
 # has_many :editors,  :class_name => 'MovieCast', :conditions => { "cast_type" => "editor" }
  ...
  def self.update_all_counters
    Movie.latest.all.each do |movie|
      ...
      hash = {:tweets_count => movie.tweets.reviews.pos_or_neg.co, :facebook_feeds_count => movie.facebook_feeds.count, :movie_casts_count => movie.movie_casts.count, :critics_reviews_count => movie.critics_reviews.count, :reviews_count => movie.reviews.count}
      Movie.update_counters movie.id, hash
    end
  end
